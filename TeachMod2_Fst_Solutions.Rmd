...
Fst_Ace_plot = ggplot(fst.summary, 
  ...
  scale_y_continuous( name = "Fst_vs_baseline",
                      breaks=seq(min_left_y, max_left_y, 0.01), 
                      limits = c(0, 0.05))+
...
# 7 Data for Rld region

We will fist subset the data for the biallelic SNPs in the extended Rld region (Chr3L:9138938 -9175249) both in the baseline and the evolving populations.

## 7.1 Subset baseline data for Rld region
```{r}
# we first bind sites and afmat to filter sites of interest in the Ace region from the data
Rdl.base = cbind(sites.base,afmat.base) %>%
    
    #filter Rld region (3L:9,048,673..9,085,239)
    filter(chrom == "3L" & pos %in% c(9138938:9175249)) 

# we select first two columns of the df for the sites
sites.Rdl.base = Rdl.base[,1:2]

# we select the rest of the columns of the columns for allele frequencies
afmat.Rdl.base =Rdl.base[,3:length(Rdl.base)]

# we don't have to subset the samples
samps.Rdl.base = samps.base
```

## 5.2 Subset evolving data for Rdl region

```{r}
# we first bind sites and afmat to filter sites of interest in the Rdl region from the data
Rdl.evolving = cbind(sites,afmat) %>%
    
   #filter Rld region (3L:9,048,673..9,085,239)
  filter(chrom == "3L" & pos %in% c(9138938:9175249)) 

# we select first two columns of the df for the sites
sites.Rdl.evolving = Rdl.evolving[,1:2]

# we select the rest of the columns of the columns for allele frequencies
afmat.Rdl.evolving =Rdl.evolving[,3:length(Rdl.evolving)]

# we don't have to subset the samples
samps.Rdl.evolving = samps
```
...
```{r}
# combine alle freq from baseline & timepoint data
afmat.base.evolving.Rdl = cbind(afmat.Rdl.base, afmat.Rdl.evolving) 


#calculate Fst between baseline samples and evolving samples
fst.base.evolving.Rdl =Fst.mat(afmat.base.evolving.Rdl) 


# turn matrix into dataframe to be able to work with dataframe, 
# remove first 4 rows of Fst between baseline samples 
# keep first 4 columns for comparison between each timepoint sample with each of the baseline samples
fst.base.evolving.Rdl.df = as.data.frame(fst.base.evolving.Rdl[-1:-4,1:4]) %>%
  ...
  # add sample information
  left_join(samps.Rdl.evolving, by = "full.sample.name") %>% 
...
```{r}
fst.summary.Rdl = fst.base.evolving.Rdl.df %>%
...
Fst_Rdl_plot = ggplot(fst.summary.Rdl, 
                      aes(x=tpt, y=mean_fst, color=treatment)) + 
